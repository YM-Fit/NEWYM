...
export default function WorkoutPlanTable({
  ...
  const getDayStatus = (dayId: string): 'completed' | 'in_progress' | 'not_started' => {
    ...
    const completedCount = getCompletedCount(dayId);
                              ...
                              return (
                                <div
                                  ...
                                  className={`p-4 rounded-xl border-2 transition-all ${
                                    isCompleted ? 'border-emerald-500/50 bg-emerald-500/10' : 'border-[var(--color-border)] bg-[var(--color-bg-base)]'
                                  }`}
                                >
                                  <div className="flex items-start gap-4">
                                    <button
                                      onClick={(e) => {
                                        e.stopPropagation();
                                        onToggleExerciseComplete(exercise.id);
                                      }}
                                      className={`flex-shrink-0 w-10 h-10 rounded-xl flex items-center justify-center transition-all ${
                                        isCompleted
                                          ? 'bg-gradient-to-br from-emerald-500 to-teal-600 text-foreground'
                                    ...
                                    <div className="flex-1 min-w-0">
                                      <div className="flex items-start justify-between mb-2">
                                        <div>
                                          <h4 className={`font-bold ${isCompleted ? 'text-emerald-400' : 'text-[var(--color-text-primary)]'}`}>
                                            {exerciseName}
                                          </h4>
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-start gap-3">
                          ...
                          >
                            {isCompleted ? <Check className="w-4 h-4" /> : <span className="text-xs font-bold">{exIndex + 1}</span>}
                          </button>
                          <div className="flex-1">
                            <div className="flex items-start justify-between mb-2">
                              <h4 className={`font-bold text-sm ${isCompleted ? 'text-emerald-400' : 'text-[var(--color-text-primary)]'}`}>
                                {exerciseName}
                              </h4>
                              {editingExercise !== exercise.id && (
