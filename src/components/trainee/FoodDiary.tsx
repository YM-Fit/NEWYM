...
export default function FoodDiary({ traineeId }: FoodDiaryProps) {
  ...
  const renderMonthView = () => {
    return (
      <div className="premium-card-static bg-white(--color-bg-elevated)] p-4">
        <div className="grid grid-cols-7 gap-2">
          {/* Week days header */}
          {weekDays.map((day, index) => (
            <div
              key={index}
              className="text-center text-sm font-semibold text-muted600(--color-text-muted)] py-2"
            >
              {day}
            </div>
          ))}
          ...
          {monthDates.map((date, index) => {
            ...
            return (
              <div
                ...
                className={`min-h-[80px] p-2 border border-border200(--color-border)]/30 rounded-lg cursor-pointer transition-all hover:border-emerald-500/50 ${
                  isCurrentDay
                    ? 'bg-emerald-500/10 border-emerald-500/30'
                    : hasMeals
                    ? 'bg-surface50(--color-bg-surface)]'
                    : 'bg-transparent'
                }`}
              >
                <div
                  className={`text-sm font-semibold mb-1 ${
                    isCurrentDay ? 'text-emerald-600' : 'text-muted900(--color-text-primary)]'
                  }`}
                >
                  {date.getDate()}
                </div>
  ...
  const renderDayContent = (
    ...
    return (
      <>
        <div
          className={`px-4 py-4 rounded-t-2xl transition-all duration-300 ${
            isToday(date) 
              ...
              : 'bg-gradient-to-br from-gray-50 to-gray-100(--color-bg-surface)](--color-bg-elevated)] border-b border-border200(--color-border)]/20'
          }`}
        >
            ...
            <div className="flex items-center gap-2">
              ...
              <div>
                <span className={`font-bold text-lg ${
                  isToday(date) ? 'text-foreground' : 'text-muted900(--color-text-primary)]'
                }`}>
                  יום {getHebrewDayName(date)}
                </span>
                <span className={`text-sm mr-2 ${
                  isToday(date) ? 'text-foreground/90' : 'text-muted600(--color-text-muted)]'
                }`}>
                  {formatDate(date)}
                </span>
                {isToday(date) && (
            ...
            <div className="flex gap-2">
              ...
              <button
                ...
                className={`p-2 rounded-xl transition-all duration-300 ${
                  isCompleted
                    ? 'opacity-50 cursor-not-allowed bg-surface100(--color-bg-surface)] text-muted400(--color-text-muted)]'
                    : isToday(date)
                    ? 'bg-white/20 hover:bg-white/30 text-foreground'
        ...
        <div className="p-4 border-b bg-gradient-to-br from-blue-50/80 to-cyan-50/80 border-border200(--color-border)]/30">
          <div className="flex items-center gap-3 mb-3">
            ...
            <div className="flex-1 min-w-0">
              <span className="text-sm font-bold text-muted900(--color-text-primary)] block">מעקב מים</span>
              <p className="text-sm text-muted600(--color-text-muted)]">
                {waterAmount} / {waterGoal} מ"ל
              </p>
            </div>
          </div>
          <div className="h-3 bg-surface200(--color-bg-surface)] rounded-full overflow-hidden mb-4 shadow-inner border border-border300(--color-border)]/30">
            <div
        ...
        {dayMeals.length > 0 && (
          <div className="px-4 py-3 border-b border-border200(--color-border)]/30 bg-gradient-to-br from-emerald-50/50 to-teal-50/50">
            <button
            ...
            >
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm font-bold text-muted900(--color-text-primary)]">סיכום יומי</span>
              </div>
              {showDailySummary.has(dateStr) ? (
                <ChevronUp className="w-4 h-4 text-muted600(--color-text-muted)]" />
              ) : (
                <ChevronDown className="w-4 h-4 text-muted600(--color-text-muted)]" />
              )}
            </button>
            {showDailySummary.has(dateStr) && (
        ...
        <div className="p-4 bg-white(--color-bg-base)]">
          {dayMeals.length === 0 ? (
            <div className="text-center py-8">
              <div className="w-16 h-16 mx-auto mb-4 rounded-full bg-surface100(--color-bg-surface)] flex items-center justify-center">
                <Utensils className="w-8 h-8 text-muted400(--color-text-muted)]" />
              </div>
              <p className="text-muted600(--color-text-muted)] text-sm mb-4">לא נרשמו ארוחות</p>
            ...
            <div className="space-y-3">
              {dayMeals.map((meal) => {
                ...
                return (
                  <div
                    key={meal.id}
                    className="border-2 border-border200(--color-border)]/30 rounded-xl p-4 hover:border-emerald-500/50 transition-all duration-300 hover:shadow-lg bg-white(--color-bg-base)] shadow-sm"
                  >
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-1 flex-wrap">
                          <span className="font-bold text-sm text-muted900(--color-text-primary)]">
                            {typeInfo.label}
                          </span>
                          {meal.meal_time && (
                            <span className="text-xs text-muted600(--color-text-muted)] flex items-center gap-1 bg-surface100(--color-bg-surface)] px-2 py-0.5 rounded-full border border-border300(--color-border)]/30">
                        ...
                        <p
                          className={`text-sm text-muted700(--color-text-secondary)] leading-relaxed ${
                      ...
                      {!isCompleted && (
                        <div className="flex gap-1">
                          <button
                            ...
                            className="p-2 text-muted600(--color-text-muted)] hover:text-emerald-600 hover:bg-emerald-50 rounded-xl transition-all duration-300"
                          >
                            <svg
                              className="w-4 h-4"
                              fill="none"
                          ...
                          <button
                            ...
                            className="p-2 text-muted600(--color-text-muted)] hover:text-red-600 hover:bg-red-50 rounded-xl transition-all duration-300"
                          >
  ...
  return (
    <div className="space-y-4 pb-4">
      <div className="premium-card-static rounded-2xl p-6 shadow-xl sticky top-0 z-10 bg-gradient-to-br from-emerald-500 to-teal-600">
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4 animate-fade-in">
          <div className="premium-card-static w-full max-w-md max-h-[90vh] overflow-y-auto bg-white(--color-bg-elevated)] shadow-2xl animate-scale-in">
            <div className="sticky top-0 bg-gradient-to-br from-emerald-500 to-teal-600 p-4 flex items-center justify-between rounded-t-2xl z-10">
              ...
              <button
                ...
                className="p-2 hover:bg-white/20 rounded-xl transition-all duration-300 text-foreground"
              >
                <X className="w-5 h-5" />
              </button>
            </div>

            <div className="p-6 space-y-6 bg-white(--color-bg-elevated)]">
              {mealPlanMeals.length > 0 && !editingMeal && (
                <div>
                  <label className="block text-sm font-bold text-muted900(--color-text-secondary)] mb-2">
                    העתק מתפריט
                  </label>
                  <div className="grid grid-cols-1 gap-2 max-h-32 overflow-y-auto">
                    {mealPlanMeals.slice(0, 3).map((planMeal) => (
                      ...
                      >
                        <div className="text-sm font-bold text-muted900(--color-text-primary)]">
                        ...
                        <div className="text-xs text-muted600(--color-text-muted)] line-clamp-1">
                      ...
                      <button
                        ...
                        className={`p-4 rounded-xl border-2 flex items-center gap-2 transition-all duration-300 ${
                          mealForm.meal_type === type.value
                            ...
                            : 'border-border300(--color-border)] hover:border-emerald-500/50 hover:shadow-md bg-white(--color-bg-base)]'
                        }`}
                      >
                        <div className={`p-2 rounded-xl ${type.color} shadow-sm`}>
                          <Icon className="w-4 h-4" />
                        </div>
                        <span className="text-sm font-bold text-muted900(--color-text-primary)]">{type.label}</span>
                      </button>
                    );
                  })}
                </div>
              ...
              <div>
                <label className="block text-sm font-bold text-muted900(--color-text-secondary)] mb-3">
                  ערכים תזונתיים (אופציונלי)
                </label>
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="block text-xs text-muted600(--color-text-muted)] mb-1">
            ...
            <div className="sticky bottom-0 bg-white(--color-bg-base)] p-4 border-t border-border200(--color-border)] flex gap-3">
              <button
                onClick={() => setShowModal(false)}
      ...
      {showMealPlanCopy && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4 animate-fade-in">
          <div className="premium-card-static w-full max-w-md max-h-[90vh] overflow-y-auto bg-white(--color-bg-elevated)] shadow-2xl animate-scale-in">
            ...
            <div className="p-6 space-y-3 bg-white(--color-bg-elevated)]">
              {mealPlanMeals.map((planMeal) => (
                ...
                >
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <span className="text-xs text-muted600(--color-text-muted)]">
                      {planMeal.meal_time}
                    </span>
                  </div>
                  <div className="text-xs text-muted700(--color-text-secondary)] mb-2">
                    {planMeal.description}
                  </div>
            ...
            <div className="sticky bottom-0 bg-white(--color-bg-base)] p-4 border-t border-border200(--color-border)]">
              <button
                onClick={() => {
                  setShowMealPlanCopy(false);
                  setShowModal(true);
    ...
    <div className="mt-4 space-y-3 animate-fade-in">
      <div className="grid grid-cols-2 gap-3">
        <div className="bg-amber-100 border border-amber-300 rounded-lg p-3 text-center shadow-sm">
          ...
          <p className="text-lg font-bold text-muted900(--color-text-primary)]">{totals.calories}</p>
          <p className="text-xs text-muted700(--color-text-secondary)]">קלוריות</p>
        ...
        <div className="bg-red-100 border border-red-300 rounded-lg p-3 text-center shadow-sm">
          ...
          <p className="text-lg font-bold text-muted900(--color-text-primary)]">{totals.protein}ג'</p>
          <p className="text-xs text-muted700(--color-text-secondary)]">חלבון</p>
        ...
        <div className="bg-amber-100 border border-amber-300 rounded-lg p-3 text-center shadow-sm">
          ...
          <p className="text-lg font-bold text-muted900(--color-text-primary)]">{totals.carbs}ג'</p>
          <p className="text-xs text-muted700(--color-text-secondary)]">פחמימות</p>
          {mealPlan?.carbs_grams && (
            <p className="text-xs text-muted600(--color-text-muted)] mt-1">מתוך {mealPlan.carbs_grams}ג'</p>
          )}
        </div>
        <div className="bg-blue-100 border border-blue-300 rounded-lg p-3 text-center shadow-sm">
          ...
          <p className="text-lg font-bold text-muted900(--color-text-primary)]">{totals.fat}ג'</p>
          <p className="text-xs text-muted700(--color-text-secondary)]">שומן</p>
          {mealPlan?.fat_grams && (
            <p className="text-xs text-muted600(--color-text-muted)] mt-1">מתוך {mealPlan.fat_grams}ג'</p>
          )}
        </div>
      </div>
      {mealPlan && (
