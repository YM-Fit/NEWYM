...
const sizeStyles: Record<ButtonSize, string> = {
  sm: 'px-3 py-1.5 text-sm rounded-xl min-h-[36px]',      // Standardized border radius
  md: 'px-4 py-2.5 text-base rounded-xl min-h-[44px]',    // Standardized border radius
  lg: 'px-6 py-3.5 text-lg rounded-xl min-h-[52px]',      // Standardized border radius
};
  ...
  ({ variant = 'primary', size = 'md', loading, icon, children, className = '', disabled, ripple = true, onClick, 'aria-label': ariaLabel, ...props }, ref) => {
    ...
    return (
      <button
        ...
        className={`
          relative inline-flex items-center justify-center gap-2 font-semibold transition-all duration-250
