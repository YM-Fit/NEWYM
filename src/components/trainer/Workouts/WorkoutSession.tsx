...
export default function WorkoutSession({
  ...
  const autoSaveWorkout = useCallback(async () => {
    // CRITICAL: Don't auto-save if we're still loading/adding an exercise
    // This prevents race conditions where auto-save triggers before exercise is fully added
    if (!user || !workoutId || exercises.length === 0 || saving || creatingWorkout || loadingExercise) return;
  ...
  }, [user, workoutId, exercises, workoutType, notes, workoutDate, trainee, selectedMember, saving, creatingWorkout, loadingExercise, setExercises]);
  ...
  useEffect(() => {
    // Don't trigger auto-save while loading/adding an exercise
    if (!workoutId || exercises.length === 0 || saving || creatingWorkout || loadingExercise) return;
  ...
  }, [exercises, workoutId, saving, creatingWorkout, loadingExercise]);

  // Helper function to find the active exercise and set (first non-minimized exercise with first non-collapsed set)
  const findActiveExerciseAndSet = useCallback((): { exerciseIndex: number; setIndex: number } | null => {
    for (let i = 0; i < exercises.length; i++) {
      if (!minimizedExercises.includes(exercises[i].tempId)) {
        ...
        // If no non-collapsed set found, return the first set
        if (exercise.sets.length > 0) {
          return { exerciseIndex: i, setIndex: 0 };
        }
      }
    }
    // Fallback to first exercise if available
        ...
        <>
          ...
          {exercises.length > 0 && exercises[0].sets.length > 0 && (
            <div className="fixed bottom-6 right-6 z-40 flex flex-col gap-3 animate-fade-in">
              ...
              {/* Quick Sets - Add set to active exercise */}
              <button
                type="button"
                onClick={() => {
                  const active = findActiveExerciseAndSet();
                  if (active) {
                    addSet(active.exerciseIndex);
                    toast.success('סט חדש נוסף', { duration: 1500, position: 'bottom-center' });
                  }
                }}
