...
export default function GoogleCalendarSettings({ onClose }: GoogleCalendarSettingsProps) {
  ...
  if (loading && !connected) {
    return (
      <div className="premium-card-static bg-white(--color-bg-elevated)] rounded-2xl shadow-xl p-6 transition-all duration-300">
        <div className="flex items-center justify-center py-8">
          <div className="flex flex-col items-center gap-4">
            ...
            <span className="text-sm font-medium text-muted600(--color-text-muted)]">טוען הגדרות...</span>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="premium-card-static bg-white(--color-bg-elevated)] rounded-2xl shadow-xl p-6 space-y-6 transition-all duration-300">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-xl font-bold text-muted900(--color-text-primary)]">הגדרות Google Calendar</h2>
            <p className="text-sm text-muted600(--color-text-muted)]">סנכרון אימונים עם Google Calendar</p>
          </div>
        </div>
        {onClose && (
          <button
            onClick={onClose}
            className="p-2 text-muted600(--color-text-muted)] hover:text-muted900(--color-text-primary)] hover:bg-surface100(--color-bg-surface)] rounded-xl transition-all duration-300"
          >
      ...
      {!connected ? (
        <div className="text-center py-8 space-y-4">
          <div className="mx-auto w-16 h-16 bg-gradient-to-br from-gray-200 to-gray-300(--color-bg-surface)](--color-bg-elevated)] rounded-2xl flex items-center justify-center shadow-lg border-2 border-border300(--color-border)]/30">
            <Calendar className="h-8 w-8 text-muted500(--color-text-muted)]" />
          </div>
          <div>
            <h3 className="text-lg font-semibold text-muted900(--color-text-primary)] mb-2">
              Google Calendar לא מחובר
            </h3>
            <p className="text-sm text-muted600(--color-text-muted)] mb-6">
              חבר את Google Calendar כדי לסנכרן אימונים אוטומטית
            </p>
          </div>
          {/* Debug: Show trainer ID (remove in production) */}
          {user && (
            <div className="text-xs text-muted500(--color-text-muted)] mb-4 px-4 py-2 bg-surface100(--color-bg-surface)] rounded-lg border border-border200(--color-border)]/30">
              Trainer ID: <code className="text-muted700(--color-text-primary)]">{user.id}</code>
            </div>
          )}
          <button
        ...
        <div className="space-y-4">
          ...
          <div className="flex items-center justify-between p-4 bg-gradient-to-br from-emerald-50 to-teal-50 rounded-xl border border-emerald-200 transition-all duration-300">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <span className="text-xs text-muted600(--color-text-muted)]">Google Calendar פעיל</span>
              </div>
            </div>
            <button
          ...
          {/* Sync Settings */}
          <div className="p-4 bg-surface50(--color-bg-surface)] rounded-xl space-y-4 border border-border200(--color-border)]/30 transition-all duration-300">
            <div className="flex items-center justify-between">
              <span className="text-sm font-semibold text-muted900(--color-text-primary)]">הגדרות סנכרון</span>
            </div>
          ...
          <button
            ...
            className="w-full btn-secondary flex items-center justify-center gap-2 bg-surface100(--color-bg-surface)] hover:bg-surface200(--color-bg-elevated)] text-muted700(--color-text-primary)] border border-border200(--color-border)]/30"
          >
            {syncing ? (
              <>
                <RefreshCw className="h-4 w-4 animate-spin" />
          ...
          <div className="flex items-start gap-3 p-4 bg-gradient-to-br from-blue-50 to-cyan-50 rounded-xl border border-blue-200 transition-all duration-300">
            ...
            <div className="text-sm text-muted700(--color-text-primary)]">
              <p className="font-semibold mb-1">מידע חשוב:</p>
              <ul className="list-disc list-inside space-y-1 text-muted600(--color-text-muted)]">
