    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          <button
            onClick={onBack}
            className="p-2.5 text-zinc-400 hover:text-white hover:bg-zinc-800 rounded-xl transition-all border border-zinc-800"
          >
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-2xl font-bold text-white">ניהול אירובי</h2>
        ...
        <button
          ...
          className="px-4 py-2.5 bg-gradient-to-r from-sky-500 to-sky-600 text-white rounded-xl font-medium hover:from-sky-600 hover:to-sky-700 transition-all flex items-center gap-2 shadow-glow"
        >
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <div className="p-4 bg-zinc-900/50 rounded-xl border border-zinc-800">
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-xs text-zinc-500">יעד צעדים שבועי</span>
              </div>
            ...
            <div className="p-4 bg-zinc-900/50 rounded-xl border border-zinc-800">
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-xs text-zinc-500">ביצוע בפועל</span>
              </div>
            ...
            <div className="p-4 bg-zinc-900/50 rounded-xl border border-zinc-800">
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-xs text-zinc-500">אחוז עמידה ביעד</span>
              </div>
            ...
            <div className="grid grid-cols-3 gap-4 mt-4 pt-4 border-t border-zinc-800">
              {latestActivity.frequency > 0 && (
                <div className="text-center">
                  <p className="text-xs text-zinc-500 mb-1">תדירות שבועית</p>
                  <p className="text-lg font-semibold text-white">{latestActivity.frequency}x</p>
                </div>
              )}
              {latestActivity.duration > 0 && (
                <div className="text-center">
                  <p className="text-xs text-zinc-500 mb-1">משך זמן</p>
                  <p className="text-lg font-semibold text-white">{latestActivity.duration} דק'</p>
                </div>
              )}
              {latestActivity.distance > 0 && (
                <div className="text-center">
                  <p className="text-xs text-zinc-500 mb-1">מרחק</p>
                  <p className="text-lg font-semibold text-white">{latestActivity.distance} ק"מ</p>
                </div>
              )}
            </div>
          )}
      ...
      {stats && (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          ...
          <div className="stat-card p-5 bg-gradient-to-br from-emerald-500/20 to-emerald-500/5">
            <div className="flex items-start justify-between mb-2">
              <div>
                <p className="text-xs font-medium text-zinc-400 mb-1">עמידה ביעד</p>
          ...
          <div className="stat-card p-5 bg-gradient-to-br from-amber-500/20 to-amber-500/5">
            <div className="flex items-start justify-between mb-2">
              <div>
                <p className="text-xs font-medium text-zinc-400 mb-1">יעד ממוצע</p>
      ...
      <div className="premium-card-static p-6">
        ...
        {activities.length === 0 ? (
          <div className="text-center py-12">
            ...
            <p className="text-zinc-500">אין עדיין פעילויות אירוביות</p>
            <button
                ...
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                      <div>
                        <p className="text-xs text-zinc-500">יעד</p>
                        <p className="text-sm font-semibold text-emerald-400">
                          {activity.weekly_goal_steps.toLocaleString()}
                        </p>
                      </div>
                      <div>
                        <p className="text-xs text-zinc-500">בוצע</p>
                        <p className="text-sm font-semibold text-amber-400">
                          {activity.avg_weekly_steps.toLocaleString()}
                        </p>
                      </div>
                      <div>
                        <p className="text-xs text-zinc-500">עמידה ביעד</p>
                    ...
                    {activity.notes && (
                      <p className="text-sm text-zinc-400 mt-2 pt-2 border-t border-zinc-800">
                        {activity.notes}
                      </p>
                    )}
                  </div>
                  ...
                  <div className="flex gap-2 mr-4">
                    <button
                      ...
                      className="p-2 text-zinc-400 hover:text-sky-400 hover:bg-sky-500/10 rounded-lg transition-all"
                    >
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="glass-card p-6 max-w-lg w-full max-h-[90vh] overflow-y-auto">
            ...
            <div className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-zinc-300 mb-2">סוג אירובי</label>
                {!showNewType ? (
                  <div className="flex gap-2">
                    <select
                      value={formData.cardio_type_id}
                      onChange={(e) => setFormData({ ...formData, cardio_type_id: e.target.value })}
                      className="flex-1 px-4 py-3 bg-zinc-900/50 border border-zinc-700 rounded-xl text-white focus:outline-none focus:ring-2 focus:ring-sky-500"
                    >
                  ...
                  <div className="flex gap-2">
                    ...
                    <button onClick={() => { setShowNewType(false); setNewCardioType(''); }} className="px-3 py-2 bg-zinc-800 text-zinc-400 rounded-xl">ביטול</button>
                  </div>
                )}
              </div>

              <div>
                <label className="block text-sm font-medium text-zinc-300 mb-2">תאריך</label>
                <input
                  type="date"
                  value={formData.date}
                  onChange={(e) => setFormData({ ...formData, date: e.target.value })}
                  className="w-full px-4 py-3 bg-zinc-900/50 border border-zinc-700 rounded-xl text-white focus:outline-none focus:ring-2 focus:ring-sky-500"
                />
              </div>

              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-zinc-300 mb-2">יעד צעדים שבועי</label>
                  <input
                    type="number"
                ...
                <div>
                  <label className="block text-sm font-medium text-zinc-300 mb-2">ביצוע בפועל</label>
                  <input
                    type="number"
                    value={formData.avg_weekly_steps || ''}
                    onChange={(e) => setFormData({ ...formData, avg_weekly_steps: parseInt(e.target.value) || 0 })}
                    className="w-full px-4 py-3 bg-zinc-900/50 border border-zinc-700 rounded-xl text-white focus:outline-none focus:ring-2 focus:ring-amber-500"
                  />
                </div>
              </div>
              ...
              <div className="grid grid-cols-3 gap-4">
                <div>
                  <label className="block text-sm font-medium text-zinc-300 mb-2">תדירות שבועית</label>
                  <input
                    type="number"
                ...
                <div>
                  <label className="block text-sm font-medium text-zinc-300 mb-2">משך (דקות)</label>
                  <input
                    type="number"
                ...
                <div>
                  <label className="block text-sm font-medium text-zinc-300 mb-2">מרחק (ק"מ)</label>
                  <input
                    type="number"
                    step="0.1"
              ...
              <div>
                <label className="block text-sm font-medium text-zinc-300 mb-2">הערות</label>
                <textarea
                  value={formData.notes}
                  onChange={(e) => setFormData({ ...formData, notes: e.target.value })}
                  className="w-full px-4 py-3 bg-zinc-900/50 border border-zinc-700 rounded-xl text-white focus:outline-none focus:ring-2 focus:ring-sky-500 resize-none"
                  rows={2}
                />
              </div>
