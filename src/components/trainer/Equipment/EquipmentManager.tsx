      ...
      <div className="bg-gradient-to-br from-gray-800/90 to-gray-900/90 rounded-2xl shadow-xl border border-white/10 p-6 lg:p-8 mb-8 backdrop-blur-sm">
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-4">
            <div className="p-4 bg-gradient-to-br from-primary-500/30 to-primary-600/30 rounded-2xl shadow-lg">
          ...
          {categories.map(cat => (
            <button
              ...
              className={`px-5 py-2.5 rounded-xl whitespace-nowrap text-sm font-semibold transition-all duration-300 ${
                filter === cat.id
                  ? 'bg-gradient-to-r from-primary-500 to-primary-600 text-white shadow-lg shadow-primary-500/25 hover:shadow-xl hover:shadow-primary-500/30 scale-105'
        ...
        {Object.entries(groupedEquipment).map(([category, items]) => (
          <div key={category} className="bg-gradient-to-br from-gray-800/90 to-gray-900/90 rounded-2xl shadow-xl border border-white/10 p-6 lg:p-8 backdrop-blur-sm transition-all duration-300 hover:shadow-2xl">
            <div className="flex items-center gap-3 mb-6">
              <div className="p-2 bg-gradient-to-br from-primary-600/20 to-primary-500/20 rounded-xl">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {items.map(item => (
                <div
                  key={item.id}
                  className="group border-2 border-border700/50 rounded-2xl p-5 bg-surface800/30 hover:border-primary-500/50 hover:bg-gradient-to-br hover:from-primary-500/5 hover:to-primary-600/5 transition-all duration-300 hover:shadow-xl hover:shadow-primary-500/10 hover:scale-[1.02]"
                >
                  <div className="flex items-start justify-between">
                    <div className="flex items-center space-x-3 rtl:space-x-reverse">
                      <span className="text-3xl filter drop-shadow-lg">{item.emoji}</span>
                      <div>
                        <h3 className="font-semibold text-white group-hover:text-primary-300 transition-colors duration-300">{item.name}</h3>
                        <div className="flex flex-wrap gap-2 mt-2">
                          ...
                          {item.is_bodyweight && (
                            <span className="text-xs text-primary-400 bg-primary-500/20 px-2.5 py-1 rounded-lg font-medium">
                              Bodyweight
                            </span>
                          )}
                        </div>
