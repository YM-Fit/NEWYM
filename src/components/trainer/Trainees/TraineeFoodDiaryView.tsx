...
export default function TraineeFoodDiaryView({ traineeId, traineeName, onBack }: TraineeFoodDiaryViewProps) {
  ...
  if (loading) {
    return (
      <div className="flex justify-center items-center py-12 min-h-screen bg-gradient-to-br from-gray-50 to-gray-100(--color-bg-base)](--color-bg-elevated)]">
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100(--color-bg-base)](--color-bg-elevated)] p-4 lg:p-6">
      {/* Premium Header */}
      <div className="premium-card-static bg-white(--color-bg-elevated)] rounded-2xl shadow-xl p-4 lg:p-6 mb-4 lg:mb-6 sticky top-0 z-10 transition-all duration-300">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center space-x-3 rtl:space-x-reverse">
            <button
              onClick={onBack}
              className="p-3 lg:p-4 hover:bg-surface(--color-bg-surface)] rounded-xl transition-all duration-300"
            >
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-base lg:text-lg text-secondary(--color-text-secondary)]">{traineeName}</p>
              </div>
            </div>
          </div>
          {unseenCount > 0 && (
            <div className="bg-gradient-to-br from-red-500 to-rose-600 text-inverse px-4 py-2 rounded-full text-sm font-bold shadow-lg transition-all duration-300 hover:scale-105">
              {unseenCount} חדשים
            </div>
          )}
        </div>
        ...
        <div className="flex items-center justify-between mb-4 bg-gradient-to-br from-gray-50 to-gray-100 rounded-xl p-3 transition-all duration-300 border border-border">
          <button
            ...
            className="p-3 hover:bg-white(--color-bg-surface)] rounded-xl transition-all duration-300 shadow-sm hover:shadow-md"
          >
        ...
        <div className="flex gap-2 mb-4">
          <button
            ...
            className={`flex-1 py-3 px-4 rounded-xl text-sm font-semibold transition-all duration-300 flex items-center justify-center gap-2 ${
              viewMode === 'day'
                ? 'bg-gradient-to-br from-emerald-500 to-teal-600 text-inverse shadow-lg'
                : 'bg-surface text-secondary hover:bg-surface(--color-bg-elevated)]'
            }`}
          >
        ...
        <div className="flex gap-2">
          ...
          >
            לא נצפו
            {unseenCount > 0 && (
              <span className="absolute -top-2 -left-2 bg-gradient-to-br from-red-500 to-rose-600 text-inverse text-xs rounded-full w-6 h-6 flex items-center justify-center font-bold shadow-md">
                {unseenCount}
              </span>
            )}
          </button>
        ...
        <div className="space-y-4 pb-4">
          {(() => {
            ...
            if (!shouldShowDay(dateStr)) {
              return (
                <div className="premium-card-static bg-white(--color-bg-elevated)] rounded-2xl shadow-xl p-10 text-center">
                  <div className="w-16 h-16 mx-auto mb-4 rounded-full bg-surface flex items-center justify-center">
            ...
            return (
              <div
                key={dateStr}
                className={`premium-card-static bg-white(--color-bg-elevated)] rounded-2xl shadow-xl overflow-hidden transition-all duration-300 hover:shadow-2xl ${
      ...
      {viewMode === 'month' && (
        <div className="premium-card-static bg-white(--color-bg-elevated)] rounded-2xl shadow-xl p-4">
      ...
      {viewMode === 'week' && weekDates.filter((date) => shouldShowDay(date.toISOString().split('T')[0])).length === 0 && (
        <div className="premium-card-static bg-white(--color-bg-elevated)] rounded-2xl shadow-xl p-10 text-center transition-all duration-300">
