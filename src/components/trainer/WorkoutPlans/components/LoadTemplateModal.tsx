  ...
  return (
    <div className="fixed inset-0 backdrop-blur-sm bg-black/60 flex items-center justify-center z-50 p-4 transition-all duration-300">
      <div className="premium-card-static max-w-2xl w-full max-h-[80vh] overflow-hidden flex flex-col transition-all duration-300">
          ...
          {templates.length === 0 ? (
            <div className="text-center py-12">
              <div className="w-20 h-20 bg-gradient-to-br from-gray-200 to-gray-300 rounded-2xl flex items-center justify-center mx-auto mb-4">
                <Dumbbell className="w-10 h-10 text-muted400" />
              </div>
            ...
            <div className="space-y-3">
              {templates.map((template) => (
                <button
                  ...
                  className="w-full text-right p-5 bg-gradient-to-br from-gray-50 to-gray-100 hover:from-blue-50 hover:to-sky-50 border-2 border-border200 hover:border-blue-400 rounded-2xl transition-all duration-300 shadow-md hover:shadow-lg"
                >
                  <h4 className="font-bold text-muted900 text-lg">{template.name}</h4>
                  {template.description && (
                    <p className="text-sm text-muted600 mt-1">{template.description}</p>
                  )}
                  <p className="text-xs text-muted500 mt-2 font-medium">
        ...
        <div className="p-6 border-t border-border200">
          <button
            onClick={onClose}
            className="w-full py-4 bg-surface200 hover:bg-surface300 text-muted700 font-bold rounded-xl transition-all duration-300"
          >
            סגור
          </button>
        </div>
